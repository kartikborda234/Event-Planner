		...
		@for($i = 0; $i < count($data); $i++) <tr class="odd gradeX">
			...
			@for($j = 0; $j < count($event); $j++) @if($data[$i]->e_id == $event[$j]->e_id)
				...
				<td>@if($data[$i]->status == 1)
					<form action="{{ route('eac.updatesubeventstatus', [$data[$i]->s_e_id,0]) }}" method="post" style="display: inline;">
						{{csrf_field()}}
					...
					<form action="{{ route('eac.updatesubeventstatus', [$data[$i]->s_e_id,1]) }}" method="post" style="display: inline;">
						{{csrf_field()}}
				...
				<td class="col-md-2">
					...
					<form action="{{ route('eac.deletesubevent', [$data[$i]->s_e_id]) }}" method="post" style="display: inline;">
						{{csrf_field()}}
						{{ method_field('DELETE') }}
...
<div class="modal fade" id="modal-7">
	<form method="post" id="addsubevent" action="{{route('eac.addsubevent')}}">
		{{csrf_field()}}
...
<div class="modal fade" id="modal-6">
	<form method="post" id="updatesubevent" action="{{route('eac.updatesubevent')}}">
		{{csrf_field()}}
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
