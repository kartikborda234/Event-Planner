		...
		@for($i = 0; $i < count($data); $i++) <tr class="odd gradeX">
			...
			<td class="col-md-2">
				<!-- <form style="display: inline;">
					<a href="javascript:;" id="{{$data[$i]->g_id}}_{{$data[$i]->e_id}}_{{$data[$i]->e_name}}_{{$data[$i]->s_e_id}}_{{$data[$i]->s_e_name}}_{{$data[$i]->u_id}}_{{$data[$i]->f_name}} {{$data[$i]->l_name}}_{{$data[$i]->r_id}}_{{$data[$i]->r_name}}" onclick="" class="btn btn-default btn-sm btn-icon icon-left"><i class="entypo-pencil"></i>Edit</a>
				</form> &nbsp; &nbsp; -->
			...
			<div class="modal-content">
				...
				<div class="modal-body" style="overflow-y:visible;">
					<div class="row">
						...
						<div class="clearfix"></div>
						<div class="col-md-6">
							<div id="s_event" class="form-group">
...
<script>
	$(document).ready(function() {
		$("#r_id").on('click change', function() {
			if ($("#r_id option:selected").text() == 'FEC') {
				$('#s_event').hide();
			} else {
				$('#s_event').show();
			}
		});
		$("#r_id_field").on('click change', function() {
			if ($("#r_id_field option:selected").text() == 'FEC') {
				$('#u_s_event').hide();
			} else {
				$('#u_s_event').show();
			}
		...
		$("#addgroup").submit(function(e) {
			...
			if ($("#r_id option:selected").text() == 'FEC') {
				$('#s_e_id').val("");
				$("#s_e_id").after(
					'<input type="hidden" name="s_e_id" value=""/>'
				);
			} else {
				if (s_e_id == null) {
					e.preventDefault();
