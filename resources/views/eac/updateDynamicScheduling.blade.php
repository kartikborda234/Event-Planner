...
<form method="post" id="updateSchedule" action="{{route('eac.updateSchedule')}}">
	{{csrf_field()}}
	<input type="hidden" value="{{Session::get('e_id')}}" name="e_id">
	<input type="hidden" name="sched_id" value="{{$sched_id}}" />
	<input type="hidden" name="sched_details" id="sched_details" value="" />
	<table class="table table-bordered datatable" id="table-4">
		...
		<tbody id="t_body">
			@for($i = 0; $i < count($data); $i++) @if($i==0) <tr class="odd gradeX">
				<td>{{$data[$i][0]->s_e_id}}</td>
				<td>{{$data[$i][0]->s_e_name}}</td>
				<td>{{$data[$i][0]->s_e_duration}}</td>
				<td>{{$data[$i][0]->overlap}}</td>
				</tr>
				@elseif($data[$i][0]->s_e_id != $data[$i-1][0]->s_e_id)
				<tr class="odd gradeX">
					<td>{{$data[$i][0]->s_e_id}}</td>
					<td>{{$data[$i][0]->s_e_name}}</td>
					<td>{{$data[$i][0]->s_e_duration}}</td>
	...
	<div style="margin-left: 35%">
		<button type="submit" class="btn btn-primary">update Scheduling</button>
	</div>
</form>
<br />
<form method="post" action="#">
	...
	<table class="table table-bordered datatable" id="table-4">
		...
		<tbody>
			<input type="hidden" id="eventCount" value="{{count($sub_event)}}" />
			<input type="hidden" id="eventSequence" value="{{$eventSequence}}" />
			@for($i = 0; $i < count($sub_event); $i++) <tr class="odd gradeX">
				<td>{{$sub_event[$i]->s_e_id}}</td>
				<td>{{$sub_event[$i]->s_e_name}}</td>
				<td>
					<input type="checkbox" value="{{$sub_event[$i]->s_e_id}}" checked onclick="getchecked(this);" />
				</td>
				</tr>
				@endfor
		</tbody>
...
<script>
	const yourArray = [];
	if (JSON.parse($("#eventSequence").val()) !== null) {
		for (let i = 0; i < JSON.parse($("#eventSequence").val()).length; i++) {
			yourArray.push(parseInt(JSON.parse($("#eventSequence").val())[i]));
		}
	}
